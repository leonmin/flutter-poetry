...
class _TabStarState extends State<TabStar> with AutomaticKeepAliveClientMixin {
  ...
  void _fetchPoetries() async {
    ...
    //     .addAll('stars', poetries, _page);
    _provider.addAll('stars', poetries, _page);
  ...
  Future<void> _onRefresh() async {
    ...
    _fetchingMore = false;
    _fetchingAll = false;
    // Provider.of<Poetries>(context, listen: false).refresh('stars');
    _provider.refresh('stars');
    if (mounted) {
      setState(() {});
    }
    _fetchPoetries();
                      ...
                      return RefreshIndicator(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            if (index == poetries.stars.length) {
                              return Container(
                                padding: EdgeInsets.all(16.0),
                                alignment: Alignment.center,
                                child: SizedBox(
                          ...
                          itemCount: poetries.stars.length + 1,
                        ),
                      );
                    },
                  ),
